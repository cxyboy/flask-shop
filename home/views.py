...
from . import home
from app import db
from app.home.forms import LoginForm,RegisterForm,PasswordForm, ForGetForm, VerifyEmail
from app.models import User,Goods,Orders,Cart,OrdersDetail
from flask import render_template, url_for, redirect, flash, session, request,make_response,Response,Flask
from werkzeug.security import generate_password_hash
from app.decorator import user_login
from app.utlib import GetCode
from io import BytesIO


@home.route('/code/')
def get_code():
    image, code = GetCode.get_verify_code()
    # 图片以二进制形式写入
    buf = BytesIO()
    image.save(buf, 'jpeg')
...
def modify_password():
    ...
    if form.validate_on_submit():
        ...
        from app.utlib import modify_pwd
        modify_pwd(user, form.data, db)
        return "<script>alert('密码修改成功');location.href='/';</script>"
    return render_template("home/modify_password.html", form=form)


#  验证邮箱，获取验证码
@home.route('/verify_email/', methods=['GET', 'POST'])
def forget():
    form = VerifyEmail()
    if form.validate_on_submit():
        from app.utlib import send_email
        send_email(form.data['email'])
        session['email'] = form.data['email']
        return redirect(url_for('home.reset_pwd'))
    return render_template('home/verify_email.html', form=form)


#  忘记旧密码，重置密码
@home.route('/reset_pwd/',methods=['POST','GET'])
def reset_pwd():
    form = ForGetForm()
    if form.validate_on_submit():
        ...
        modify_pwd(user, form.data, db)
        return redirect(url_for('home.login'))
    return render_template('home/reset_pwd.html', form=form)
